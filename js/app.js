...
function renderList(items) {
  ...
  items.forEach((item) => {
    ...
    btn.title = item.link;
    btn.onclick = async () => {
      await withLoading(btn, "抓取全文中...", async () => {
        setStatus("正在抓取正文全文...", "loading");
        let full = "";
        try { full = await window.fetchFullArticle(item.link); } catch (_error) { full = item.summary; }
        const picked = full || item.summary || item.link;
        dom.rawInput.value = `标题：${item.title}\n链接：${item.link}\n\n${picked}`;
        setStatus("已填充全文素材", "success");
      });
    };
...
function injectImagesIntoHtml(baseHtml) {
  ...
  return sections.map((seg) => {
    if (/^<h2/i.test(seg) && imgs[idx]) {
      const imageHtml = `<p style="text-align:center;"><img src="${imgs[idx]}" style="width:100%;border-radius:12px;margin:10px 0;" /></p>`;
      idx += 1;
...
function loadDraft() {
  ...
  if (dom.aiOutput.value.trim()) {
    dom.markdownPreview.innerHTML = window.renderMarkdown(dom.aiOutput.value);
  }
}

function initEvents() {
  dom.platform.addEventListener("change", onSourcePlatformChange);
  dom.themeStyle.addEventListener("change", () => {
    document.body.className = dom.themeStyle.value;
  });
